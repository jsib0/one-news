	...
	homePage(event){
		...
		let newslist = event.articles.map( (head) => 
					<div key={head.url} className="newslist-title" >
						<a key={head.url} href={head.url} target="_blank">
						<div className="newslist-image" ><img key={head.url}src={head.urlToImage} alt=""/></div>
						<div className="newslist-header"><h4 key={head.url}>{head.title}</h4></div>
						<div className="newslist-p-tag"><p  key={head.url}>{head.description}</p></div>
						</a>
					</div>
			)

		return (
			<div key={'lksahdkahd'} className="main-container">
